...
contract BackUpContract is ERC721URIStorage {
    struct CharacterAttributes {
        uint256 characterIndex;
        string characterURI;
    }

    struct UserCharacterAttributes {
        uint256 tokenId;
        uint256 characterIndex;
        string characterURI;
    }
    ...
    mapping(address => UserCharacterAttributes[]) public nftHolders;
    ...
    function mintNFT(uint256 _characterId) public {
        ...
        nftHolders[msg.sender].push(
            UserCharacterAttributes({
                tokenId: newTokenId,
                characterIndex: _characterId,
                characterURI: defaultCharacters[_characterId].characterURI
            })
    ...
    function isOwner(uint256 _tokenId) public returns (bool) {
        // bool owner = false;
        for (uint256 i; i < nftHolders[msg.sender].length; i++) {
            if (nftHolders[msg.sender][i].tokenId == _tokenId) {
                nftHolders[msg.sender][i] = nftHolders[msg.sender][
    ...
    function getCharacterIdx(uint256 _tokenId)
        private
        view
        returns (uint256 characterIndex)
    {
        for (uint256 i; i < nftHolders[msg.sender].length; i++) {
            if (nftHolders[msg.sender][i].tokenId == _tokenId) {
                return nftHolders[msg.sender][i].characterIndex;
            }
        }
    }
    ...
    function combineNFTs(uint256 _tokenId1, uint256 _tokenId2) public {
        if (isOwner(_tokenId1) && isOwner(_tokenId2)) {
            ...
            uint256 charIdx1 = getCharacterIdx(_tokenId1);
            uint256 charIdx2 = getCharacterIdx(_tokenId2);

            transferFrom(msg.sender, address(this), _tokenId1);
            transferFrom(msg.sender, address(this), _tokenId2);
            mintNFT((charIdx1 + charIdx2) % defaultCharacters.length);
        }
    }
    ...
    function getUserNFTS()
        public
        view
        returns (UserCharacterAttributes[] memory)
    {
        if (nftHolders[msg.sender].length > 0) {
            return nftHolders[msg.sender];
        } else {
            UserCharacterAttributes[] memory emptyArray;
            return emptyArray;
        }
    }
